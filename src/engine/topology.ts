...
import {countParams} from '../utils/layer_utils';
...
export abstract class Layer extends serialization.Serializable {
  ...
  countParams(): number {
    ...
    return countParams(this.weights);
  }
...
export interface InputConfig {
   ...
   * `batchShape=[10, 32]` indicates that the expected input will be batches
   * of 10 32-dimensional vectors. `batchShape=[null, 32]` indicates batches
   * of an arbitrary number of 32-dimensional vectors.
   */
  batchShape?: Shape;
  /**
   * An optional name string for the layer. Should be unique in a model (do
...
export abstract class Container extends Layer {
  ...
  constructor(config: ContainerConfig) {
    ...
    */
    ...
    /*
      ...
      we compute the output tensors, output masks and output shapes in one
      pass, then cache them here. When one of these outputs is queried later,
   ...
   * Porting Note: TensorFlow.js Layers supports only loading by name
   * currently.
   *
   ...
   * @param mask A mask or list of masks. A mask can be either a tensor or
   * null (no mask).
   *
   * @return A tensor if there is a single output, or a list of tensors if
   ...
   * @param mask Tensor or list of tensors.
   *
   * @return null or a tensor (or list of tensors, one per output tensor of
   * the layer).
   */
  computeMask(inputs: Tensor|Tensor[], mask?: Tensor|Tensor[]): Tensor
      |Tensor[] {
