...
export class InputLayer extends Layer {
  ...
  constructor(config: InputLayerConfig) {
    // console.log('config.inputTensor:', config.inputTensor);  // DEBUG
    super({
      dtype: config.dtype,
    ...
    // console.log(`batchInputshape =`, batchInputShape);  // DEBUG
    if (batchInputShape == null) {
      if (config.inputShape == null) {
        if (config.inputTensor == null) {
          throw new ValueError(
              'An InputLayer should be passed a ' +
              '`batchInputShape`, an `inputShape` or an `inpuTensor`.');
        } else {
          batchInputShape = config.inputTensor.shape;
        }
      } else {
